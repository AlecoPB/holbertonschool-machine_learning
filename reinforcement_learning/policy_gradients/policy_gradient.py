...
def policy(matrix, weight):
    ...
    of state vectors (each row as a state).
...
def policy_gradient(state, weight):
    ...
    Parameters:
    - state: np.ndarray, the matrix representing the
    current observation of the environment.
    - weight: np.ndarray, the matrix of random weights.

    Returns:
    - tuple: (action, gradient) where action is the
    selected action and gradient is the policy gradient.
    """
