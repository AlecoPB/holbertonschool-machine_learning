...
def forward_algorithm(observations, emission_probs, transition_probs, initial_probs):
    """
    ...
    if not isinstance(emission_probs, np.ndarray) or emission_probs.ndim != 2:
        return None, None
    if not isinstance(transition_probs, np.ndarray) or transition_probs.ndim != 2:
        return None, None
    ...
    # Fill the first column of forward_probs using initial_probs and the first observation
    forward_probs[:, 0] = initial_probs[:, 0] * emission_probs[:, observations[0]]

    # Recursion to compute forward probabilities
    for t in range(1, T):
        # Calculate the next column using the previous column
        forward_probs[:, t] = forward_probs[:, t-1] @ transition_probs * emission_probs[:, observations[t]]

    # The probability of the observation sequence is the sum of the last column in forward_probs
    likelihood = np.sum(forward_probs[:, -1])

    return likelihood, forward_probs


def backward_algorithm(observations, emission_probs, transition_probs, initial_probs):
    """
    ...
    if (not isinstance(observations, np.ndarray) or observations.ndim != 1 or
            not isinstance(emission_probs, np.ndarray) or emission_probs.ndim != 2 or
            not isinstance(transition_probs, np.ndarray) or transition_probs.ndim != 2 or
            not isinstance(initial_probs, np.ndarray) or initial_probs.ndim != 2):
        return None, None
    ...
    # Set the last column of backward_probs to 1 (initial state assumed as given)
    ...
    for t in range(T - 2, -1, -1):
        ...
        backward_probs[:, t] = np.sum(
            transition_probs * emission_probs[:, observations[t + 1]] * backward_probs[:, t + 1], axis=1)

    # Calculate the likelihood of the observations given the model
    likelihood = np.sum(initial_probs[:, 0] * emission_probs[:, observations[0]] * backward_probs[:, 0])
...
def baum_welch_algorithm(observations, transition_probs, emission_probs, initial_probs, num_iterations=1000):
    ...
    for _ in range(num_iterations):
        ...
        likelihood_f, forward_probs = forward_algorithm(observations, emission_probs, transition_probs, initial_probs)
        likelihood_b, backward_probs = backward_algorithm(observations, emission_probs, transition_probs, initial_probs)
        ...
        for t in range(T-1):
            ...
            xi[:, :, t] = (forward_probs[:, t, np.newaxis] * transition_probs *
                           emission_probs[:, observations[t+1]] * backward_probs[:, t+1]) / likelihood_f

        gamma = np.sum(xi, axis=1)

        # Include the final gamma element for the new backward probabilities
        prod = (forward_probs[:, T-1] * backward_probs[:, T-1]).reshape((-1, 1))
