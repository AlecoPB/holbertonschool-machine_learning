...
class Yolo:
    ...
    def show_boxes(self, image, boxes, box_classes, box_scores, file_name):
        """
        Displays the image with bounding boxes, class names, and box scores.

        Parameters:
        - image: numpy.ndarray representing the unprocessed image
        - boxes: numpy.ndarray containing the bounding boxes for the image
        - box_classes: numpy.ndarray with the class indices for each box
        - box_scores: numpy.ndarray with the scores for each box
        - file_name: the file path where the original image is located
        """
        # Iterate through the boxes, their corresponding classes, and scores
        ...
        if key == ord('s'):
            # Save the image in the detections directory
